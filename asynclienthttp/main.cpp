...
public:
    client(boost::asio::io_context &io_context,
           ...
           std::string typereq, std::string length, std::string typecont, std::string content)
...
private:
    ...
    void handle_read_status_line(const boost::system::error_code &err) {
        if (!err) {
            ...
            if (status_code != 200 && status_code != 401 && status_code != 504 && status_code != 205) {
...
void post_method(std::string path, const std::string &auth, std::string email,
                 std::string extension) { //Creiamo il metodo per la post
    boost::asio::io_context io_context;
...
void delete_method(std::string path, const std::string &auth, std::string email,
                   std::string extension) { //Creiamo il metodo per la delete
    boost::asio::io_context io_context;
